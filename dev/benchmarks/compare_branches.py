...
def ensure_env(
    ...
    _run([str(python_bin), "-m", "pip", "install", "-U", "pip", "wheel", "setuptools"])
    _run([
        str(python_bin), "-m", "pip", "install",
        "-r", str(REPO_DPATH / "requirements" / "runtime.txt"),
        "-r", str(REPO_DPATH / "requirements" / "headless.txt"),
        "-r", str(REPO_DPATH / "requirements" / "tests.txt"),
        "matplotlib>=3.8",
    ])
    if include_gdal:
        _run([
            str(python_bin), "-m", "pip", "install",
            "-r", str(REPO_DPATH / "requirements" / "gdal.txt"),
        ])
    return python_bin
...
def build_gdal_benchmark_cases(delayed_image, kwimage, np):
    if not has_gdal():
        raise RuntimeError("GDAL benchmark cases requested, but osgeo.gdal is unavailable")
    ...
    configs = [
        {"dtype_name": "uint8", "num_channels": 3, "shape": (2048, 2048), "tag": "u8_rgb"},
        {"dtype_name": "uint8", "num_channels": 8, "shape": (2048, 2048), "tag": "u8_multi"},
        {"dtype_name": "float32", "num_channels": 3, "shape": (2048, 2048), "tag": "f32_rgb"},
        {"dtype_name": "float32", "num_channels": 8, "shape": (2048, 2048), "tag": "f32_multi"},
    ]
    ...
    for config in configs:
        ...
        for with_rotation in [False, True]:
            ...
            def scenario(base=base, patch_specs=patch_specs, with_rotation=with_rotation):
                total = 0.0
...
def build_benchmark_cases(include_gdal: bool = False):
    ...
    def optimize_deep_crop_chain():
        ...
        for idx in range(18):
            pad = idx % 4
            delayed = delayed.crop((slice(pad, 720 - pad), slice(pad, 720 - pad)))
    ...
    def finalize_nearest_noninteger_scale():
        ...
        warped = delayed.warp(kwimage.Affine.coerce(offset=(0, 0), scale=(8.6, 8.5)))
        data = warped.finalize(interpolation="nearest")
        return float(np.nanmean(data))
...
def suite_main(args):
    ...
    commit = _run(["git", "rev-parse", "HEAD"], cwd=repo_dpath, capture=True).strip()
    branch = _run(["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=repo_dpath, capture=True).strip()

    cases = build_benchmark_cases(include_gdal=args.include_gdal)
    results = [
        benchmark_case(case, warmup=args.warmup, repeats=args.repeats, min_time=args.min_time)
        for case in cases
    ]
    payload = {
        "repo": str(repo_dpath),
...
def plot_results(payloads, output_fpath: pathlib.Path, baseline_ref: str) -> None:
    import matplotlib
    matplotlib.use("Agg")
    ...
    label_map = {case["name"]: case["description"] for case in baseline["cases"]}
    baseline_times = {case["name"]: case["median_sec"] for case in baseline["cases"]}
...
def compare_main(args):
    ...
    if (
        pathlib.Path(sys.executable).resolve() != python_bin.resolve() and
        os.environ.get("DELAYED_IMAGE_BENCH_REEXEC") != "1"
    ):
        env = os.environ.copy()
        env["DELAYED_IMAGE_BENCH_REEXEC"] = "1"
        _run([str(python_bin), str(SCRIPT_FPATH), *sys.argv[1:]], cwd=REPO_DPATH, env=env)
        return
    ...
    comparison_fpath.write_text(json.dumps({
        "manifest": manifest,
        "payloads": payloads,
    }, indent=2))
    ...
    _run([
        ...
        str(SCRIPT_FPATH),
        "plot",
        "--input", str(comparison_fpath),
        "--output", str(plot_fpath),
...
def build_parser():
    ...
    compare.add_argument("--refs", nargs="+", default=None, help="git refs to compare")
    compare.add_argument("--baseline-ref", default=None, help="baseline ref for speedup ratios")
    compare.add_argument("--python", default=None, help="python executable used to build the benchmark venv")
    ...
    compare.add_argument("--include-gdal", action="store_true", help="install GDAL and include COG patch benchmarks")
    compare.set_defaults(main=compare_main)

    suite = subparsers.add_parser("suite", help="run the benchmark suite for one repo")
    suite.add_argument("--repo", required=True, help="path to the target repo/worktree")
    ...
    suite.add_argument("--include-gdal", action="store_true", help="include GDAL/COG patch benchmarks")
    suite.set_defaults(main=suite_main)

    plot = subparsers.add_parser("plot", help="plot a completed comparison json")
