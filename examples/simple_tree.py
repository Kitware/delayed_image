def demo_simple_tree():
    ...
    g = delayed_image.DelayedLoad.demo(
        channels='rgb', overviews=3, dsize=(128, 128)
    ).take_channels([1])
    b = delayed_image.DelayedLoad.demo(
        channels='rgb', overviews=3, dsize=(512, 512)
    ).take_channels([2])
    ...
    naive = rescaled.dequantize(
        {'orig_max': 1.0, 'orig_min': 0.0, 'quant_max': 255}
    )
    ...
    kwplot.autompl()
    naive_canvas = kwimage.draw_header_text(
        im_optimized, f'naive\n{t1.elapsed:0.4f}s', fit='shrink'
    )
    opt_canvas = kwimage.draw_header_text(
        im_optimized, f'optimized\n{t2.elapsed:0.4f}s', fit='shrink'
    )
    diff_canvas = kwimage.draw_header_text(
        im_diff, 'difference\nimage', fit='shrink'
    )
    stacked = kwimage.stack_images(
        [opt_canvas, naive_canvas, diff_canvas],
        axis=1,
        pad=10,
        bg_value='darkgreen',
    )
